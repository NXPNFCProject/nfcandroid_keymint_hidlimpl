...
ScopedAStatus JavacardKeyMint3Device::generateKey(const vector<KeyParameter>& keyParams,
                                                  const optional<AttestationKey>& attestationKey,
                                                  KeyCreationResult* creationResult) {
    return device_->generateKey(keyParams, attestationKey, creationResult);
}

ScopedAStatus JavacardKeyMint3Device::addRngEntropy(const vector<uint8_t>& data) {
    return device_->addRngEntropy(data);
}

ScopedAStatus JavacardKeyMint3Device::importKey(const vector<KeyParameter>& keyParams,
...
ScopedAStatus JavacardKeyMint3Device::importWrappedKey(const vector<uint8_t>& wrappedKeyData,
...
ScopedAStatus JavacardKeyMint3Device::upgradeKey(const vector<uint8_t>& keyBlobToUpgrade,
                                                 const vector<KeyParameter>& upgradeParams,
                                                 vector<uint8_t>* keyBlob) {
    return device_->upgradeKey(keyBlobToUpgrade, upgradeParams, keyBlob);
}

ScopedAStatus JavacardKeyMint3Device::deleteKey(const vector<uint8_t>& keyBlob) {
    return device_->deleteKey(keyBlob);
}

ScopedAStatus JavacardKeyMint3Device::deleteAllKeys() {
    return device_->deleteAllKeys();
}

ScopedAStatus JavacardKeyMint3Device::destroyAttestationIds() {
    return device_->destroyAttestationIds();
}

ScopedAStatus JavacardKeyMint3Device::begin(KeyPurpose purpose, const std::vector<uint8_t>& keyBlob,
...
ScopedAStatus
JavacardKeyMint3Device::deviceLocked(bool passwordOnly,
                                     const std::optional<TimeStampToken>& timestampToken) {
    return device_->deviceLocked(passwordOnly, timestampToken);
}

ScopedAStatus JavacardKeyMint3Device::earlyBootEnded() {
    return device_->earlyBootEnded();
}

ScopedAStatus JavacardKeyMint3Device::getKeyCharacteristics(
...
ScopedAStatus JavacardKeyMint3Device::convertStorageKeyToEphemeral(
    const std::vector<uint8_t>& /* storageKeyBlob */,
    std::vector<uint8_t>* /* ephemeralKeyBlob */) {
    return km_utils::kmError2ScopedAStatus(KM_ERROR_UNIMPLEMENTED);
}

ScopedAStatus JavacardKeyMint3Device::getRootOfTrustChallenge(array<uint8_t, 16>* challenge) {
    return device_->getRootOfTrustChallenge(challenge);
}

ScopedAStatus JavacardKeyMint3Device::getRootOfTrust(const array<uint8_t, 16>& /*challenge*/,
                                                     vector<uint8_t>* /*rootOfTrust*/) {
    return km_utils::kmError2ScopedAStatus(KM_ERROR_UNIMPLEMENTED);
}

ScopedAStatus JavacardKeyMint3Device::sendRootOfTrust(const vector<uint8_t>& rootOfTrust) {
    return device_->sendRootOfTrust(rootOfTrust);
}
binder_status_t JavacardKeyMint3Device::dump(int fd, const char** args, uint32_t num_args) {
